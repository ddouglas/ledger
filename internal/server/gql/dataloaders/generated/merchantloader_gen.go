...
type MerchantLoader struct {
	...
	cache map[string]*ledger.Merchant
...
func (l *MerchantLoader) Load(ctx context.Context, key string) (*ledger.Merchant, error) {
...
func (l *MerchantLoader) LoadThunk(ctx context.Context, key string) func() (*ledger.Merchant, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*ledger.Merchant, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *MerchantLoader) LoadAll(ctx context.Context, keys []string) ([]*ledger.Merchant, []error) {
	results := make([]func() (*ledger.Merchant, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(ctx, key)
	}

	merchants := make([]*ledger.Merchant, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *MerchantLoader) LoadAllThunk(ctx context.Context, keys []string) func() ([]*ledger.Merchant, []error) {
	results := make([]func() (*ledger.Merchant, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(ctx, key)
	}
...
func (l *MerchantLoader) Prime(key string, value *ledger.Merchant) bool {
	l.mu.Lock()
	var found bool
...
func (l *MerchantLoader) unsafeSet(key string, value *ledger.Merchant) {
	if l.cache == nil {
		l.cache = map[string]*ledger.Merchant{}
	}
	l.cache[key] = value
}
...
func (b *merchantLoaderBatch) keyIndex(ctx context.Context, l *MerchantLoader, key string) int {
...
func (b *merchantLoaderBatch) startTimer(ctx context.Context, l *MerchantLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *merchantLoaderBatch) end(ctx context.Context, l *MerchantLoader) {
