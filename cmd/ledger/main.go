...
func buildNewRelic() *newrelic.Application {
	...
	logger.Info("[NewRelic] Wait For Connection")
	defer logger.Info("[NewRelic] ")
	err = app.WaitForConnection(time.Second * 20)
	if err != nil {
		logger.WithError(err).Panicf("connection to newrelic failed")
	}
...
func actionAPI(c *cli.Context) error {
	...
	server := server.New(cfg.API.Port, cfg.Auth0.ServerToken, logger, auth, core.gateway, user)
